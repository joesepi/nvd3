nv.models.multiBarTimeSeries = function () {
    ...
    var margin = {
        ...
        delay = 1200,
        xDomain, yDomain, xRange, yRange, dispatch = d3.dispatch('chartClick',
    ...
    function chart(selection) {
        selection.each(function (data) {
            ...
            x.domain(xDomain || d3.extent(d3.merge(seriesData)
                .map(function (d) {
                    return d.x
                })))
            ...
            var wrap = container.selectAll('g.nv-wrap.nv-multibar')
                .data([data]);
            ...
            var barsEnter = bars.enter()
                ...
                .attr('x', function (d, i, j) {
                    return stacked ? 0 : (i * bandWidth) + (j *
                        barWidth)
                })
                .attr('y', function (d) {
            ...
            bars
                .on('mouseover', function (d, i) { //TODO: figure out why j works above, but not here
                    ...
                    dispatch.elementMouseover({
                        ...
                        pos: [x(getX(d, i)) + (barWidth * (stacked ?
                            data.length / 2 : d.series + 0.5
                        ) / data.length), y(getY(d, i) + (
            ...
            if (stacked)
                d3.transition(bars)
                    ...
                    .attr('height', function (d, i) {
                        return Math.abs(y(d.y + (stacked ? d.y0 : 0)) -
                            y((stacked ? d.y0 : 0)))
                    })
                    .each('end', function () {
                        d3.transition(d3.select(this))
                            .attr('x', function (d, i) {
                                return stacked ? 0 : (i * bandWidth) +
                                    (j * barWidth)
                            })
                            .attr('width', stacked ? bandWidth :
                                barWidth);
                    })
            else
                d3.transition(bars)
