nv.models.multiBar = function () {
    ...
    var canvas = new Canvas({
        ...
        dispatch = d3.dispatch('chartClick', 'elementClick', 'elementDblClick',
            'elementMouseover', 'elementMouseout', 'renderEnd');
    ...
    function chart(selection) {
        ...
        selection.each(function (data) {
            ...
            var endFn = function (d, i) {
                return d.series === data.length - 1 && i === data[0].values
                    .length - 1;
            };
            ...
            y.domain(yDomain || d3.extent(d3.merge(seriesData)
                .map(function (d) {
                    return stacked ? (d.y > 0 ? d.y1 : d.y1 + d.y) :
                        d.y
                })
                .concat(forceY)))
            ...
            var exitTransition = renderWatch
                .transition(groups.exit()
                    .selectAll('rect.nv-bar'), 'multibarExit', Math.min(250,
                        duration))
                .attr('y', function (d) {
            ...
            function _onMouseEventObject(d, i) {
                return {
                    ...
                    pos: [x(getX(d, i)) + (x.rangeBand() * (stacked ? data.length /
                        2 : d.series + 0.5) / data.length), y(getY(d, i) +
            ...
            var barSelection =
                bars.watchTransition(renderWatch, 'multibar', Math.min(250,
                    duration))
                .delay(function (d, i) {
                    return i * duration / data[0].values.length;
                });
            if (stacked)
                barSelection
                    ...
                    .attr('height', function (d) {
                        return Math.max(Math.abs(y(d.y + (stacked ? d.y0 :
                            0)) - y((stacked ? d.y0 : 0))), 1);
                    })
                    .attr('x', function (d) {
                        return stacked ? 0 : (d.series * x.rangeBand() /
                            data.length)
                    })
                    .attr('width', x.rangeBand() / (stacked ? 1 : data.length));
            else
