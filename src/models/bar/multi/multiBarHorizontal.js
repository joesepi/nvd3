nv.models.multiBarHorizontal = function () {
    ...
    var canvas = new Canvas({
        ...
        delay = 1200,
        xDomain, yDomain, xRange, yRange, duration,
        dispatch = d3.dispatch(
            'chartClick', 'elementClick', 'elementDblClick', 'elementMouseover',
    ...
    function chart(selection) {
        ...
        selection.each(function (data) {
            ...
            x.domain(xDomain || d3.merge(seriesData)
                .map(function (d) {
                    return d.x
                }))
            ...
            y.domain(yDomain || d3.extent(d3.merge(seriesData)
                .map(function (d) {
                    return stacked ? (d.y > 0 ? d.y1 + d.y : d.y1) :
                        d.y
                })
                .concat(forceY)));
            ...
            if (showValues && !stacked)
                y.range(yRange || [(y.domain()[0] < 0 ? valuePadding : 0),
                    availableWidth - (y.domain()[1] > 0 ? valuePadding :
                        0)]);
            else
                y.range(yRange || [0, availableWidth]);

            x0 = x0 || x;
            y0 = y0 || d3.scale.linear()
                .domain(y.domain())
                .range([y(0), y(0)]);
            ...
            var groups = canvas.wrap.select('.nv-groups')
                .selectAll('.nv-group')
                .data(function (d) {
                    return d
                }, function (d, i) {
            ...
            var barsEnter = bars
                ...
                .attr('transform', function (d, i, j) {
                    return 'translate(' + y0(stacked ? d.y0 : 0) + ',' +
                        (stacked ? 0 : (j * x.rangeBand() / data.length) +
                        x(getX(d, i))) + ')'
                });
            ...
            bars
                .on('mouseover', function (d, i) { //TODO: figure out why j works above, but not here
                    ...
                    dispatch.elementMouseover({
                        ...
                        pos: [y(getY(d, i) + (stacked ? d.y0 : 0)),
                            x(getX(d, i)) + (x.rangeBand() * (
                                stacked ? data.length / 2 : d.series +
                                0.5) / data.length)],
                        pointIndex: i,
                ...
                .on('click', function (d, i) {
                    dispatch.elementClick({
                        ...
                        point: d,
                        series: data[d.series],
                        pos: [x(getX(d, i)) + (x.rangeBand() * (
                            stacked ? data.length / 2 : d.series +
                            0.5) / data.length), y(getY(d, i) +
                            (stacked ? d.y0 : 0))], // TODO: Figure out why the value appears to be shifted
                        pointIndex: i,
                        seriesIndex: d.series,
                        e: d3.event
                    });
            ...
            if (showValues && !stacked) {
                ...
                bars.transition()
                    .select('text')
                    .attr('x', function (d, i) {
                        return getY(d, i) < 0 ? -4 : y(getY(d, i)) - y(
                            0) + 4
                    })
            } else
                bars.selectAll('text')
            ...
            if (barColor) {
                ...
                var _colorBars = function (d, i, j) {
                    return d3
                        .rgb(barColor(d, i))
                        .darker(disabled.map(function (d, i) {
                                return i
                            })
                            .filter(function (d, i) {
                                return !disabled[i]
            ...
            if (stacked)
                bars.transition()
                    .attr('transform', function (d, i) {
                        return 'translate(' + y(d.y1) + ',' + x(getX(d,
                            i)) + ')'
                    })
                    .select('rect')
                ...
                bars.transition()
                    ...
                    .attr('width', function (d, i) {
                        return Math.max(Math.abs(y(getY(d, i)) - y(0)),
                            1)
                    });
