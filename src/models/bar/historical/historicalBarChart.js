function HistoricalBarChart(options) {
    ...
    this.dispatch = d3.dispatch('tooltipShow', 'tooltipHide', 'stateChange',
        'changeState');
...
HistoricalBarChart.prototype.wrapChart = function (data) {
    ...
    this.state.disabled = data.map(function (d) {
        return !!d.disabled
    });
    ...
    var historicalBarWrap = this.g.select('.nv-barsWrap')
        .datum(data);
    d3.transition(historicalBarWrap)
        .call(this.historicalBar);
};
...
HistoricalBarChart.prototype.showTooltip = function (e, offsetElement) {
    ...
    if (offsetElement) {
        var svg = d3.select(offsetElement)
            .select('svg');
...
nv.models.historicalBarChart = function () {
    ...
    d3.rebind(chart, historicalBarChart.historicalBar, 'defined', 'isArea', 'x',
        'y', 'size', 'xScale', 'yScale',
        'xDomain', 'yDomain', 'xRange', 'yRange', 'forceX', 'forceY',
        'interactive', 'clipEdge', 'clipVoronoi',
    ...
    [
        ...
        .forEach(function (method) {
            chart[method] = function (arg1) {
                ...
                case 0:
                    ret = HistoricalBarChart.prototype[method].call(
                        historicalBarChart);
                    break;
                case 1:
                    ret = HistoricalBarChart.prototype[method].call(
                        historicalBarChart, arg1);
                    break;
                default:
                    ret = HistoricalBarChart.prototype[method].apply(
                        historicalBarChart, arguments)
                }
