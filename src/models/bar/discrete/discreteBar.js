...
nv.models.discreteBar = function () {
    ...
    function chart(selection) {
        selection.each(function (data) {
            ...
            if (showValues)
                y.range(yRange || [availableHeight - (y.domain()[0] < 0 ?
                    12 : 0), y.domain()[1] > 0 ? 12 : 0]);
            else
            ...
            function _mouseEventObject(d, i) {
                return {
                    ...
                    pos: [x(getX(d, i)) + (x.rangeBand() * (d.series + 0.5) /
                        dataLength), y(getY(d, i))], // TODO: Figure out why the value appears to be shifted
                    pointIndex: i,
            ...
            var barsEnter = bars.enter()
                ...
                .attr('transform', function (d, i) {
                    return 'translate(' + (x(getX(d, i)) + x.rangeBand() *
                        0.05) + ', ' + y(0) + ')'
                })
                .on('mouseover', function (d, i) { //TODO: figure out why j works above, but not here
                    d3.select(this)
                        .classed('hover', true);
                    dispatch.elementMouseover(_mouseEventObject(
                        arguments));
                })
                .on('mouseout', function (d, i) {
                    d3.select(this)
                        .classed('hover', false);
                    dispatch.elementMouseout(_mouseEventObject(
                        arguments));
                })
                ...
                .on('dblclick', function (d, i) {
                    dispatch.elementDblClick(_mouseEventObject(
                        arguments));
                    d3.event.stopPropagation();
                });
            ...
            barsEnter.append('rect')
                .attr('height', 0)
                .attr('width', x.rangeBand() * 0.9 / dataLength);
            ...
            if (showValues) {
                ...
                bars.select('text')
                    ...
                    .attr('y', function (d, i) {
                        return getY(d, i) < 0 ? y(getY(d, i)) - y(0) +
                            12 : -4
                    });
            } else
                bars.selectAll('text')
            ...
            bars.attr('class', function (d, i) {
                return barClass + ' ' + (getY(d, i) < 0 ? 'negative' :
                    'positive')
            })
            ...
            .attr('transform', function (d, i) {
                ...
                .attr('height', function (d, i) {
                    return Math.max(Math.abs(y(getY(d, i)) - y((yDomain &&
                        yDomain[0]) || 0)) || 1)
                });
